...
function init(page,itemsPerPage){
	...
	var url=basePath+"center/fav/favorites/findFavFavoritesPageList.do";
...
function grid(res,$e){
	...
	for(i;i<rows.length;i++){
		var row=rows[i];
		var edit='<button type="button" class="btn btn-primary btn-xs" onclick="editFavFavorites(\''+i+'\');">编辑</button>';
		if(typeof(select)!="undefined"){
			edit='<button type="button" class="btn btn-primary btn-xs" onclick="selectFavFavorites(\''+i+'\');" >选择</button>';
		}
		var data={ 
			"no":i+1,
 ...
 * 选择页时使用
 */
function selectFavFavorites(rownum){
	if(callback){
 ...
 * edit
 */
function editFavFavorites(rownum){
	var id="";
	...
	var dialog = kkdUi.dialog({
		title:'编辑',
		height : '400px'
	}, basePath+'manage/fav/favorites/opFavFavorites.jsp?id=' + id, function() {
