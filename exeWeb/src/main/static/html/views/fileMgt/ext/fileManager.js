layui.define(['jquery', 'layer', 'laypage'], function(exports) { //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
    ...
    //外部接口
    var fm = {
        config:{'test':'test','thumb':{'nopic':'',width:100,height:100},icon_url:'ico/',btn_upload:true,btn_create:true,btn_newfile:true}
        ,cache: {} //数据缓存
    ...
    Class.prototype.render = function(){
        ...
        if(options.btn_create){
 		  ...
 		  if(options.btn_newfile){
            _btn +='<button type="button" class="layui-btn layui-btn-primary layui-btn-sm" id="new_file">新建文件</button>';
        }
        ...
        layui.each(data,function(i,v){
            ...
            switch (v.type) {
                ...
                default:
                    ...
                    if (v.type == 'png' || v.type == 'gif' || v.type == 'jpg' || v.type == 'image'|| v.type == 'ico') {
    ...
    Class.prototype.events = function(){
        ...
        $("html").click(function(){
	    	$('.rightmenu').hide(); 
         $('.blank_rightmenu').hide(); 
	    });
	    
        //文件右键事件
        that.layBody.on('contextmenu', 'li', function(e){ //单击行
            $('.rightmenu').hide(); 
            $('.blank_rightmenu').hide(); 
        	//setPicEvent.call(this, 'pic');
//        	selectedItem=$(this);
        	//selectedItem = $(this);
            fm.cache["selectedItem"]=$(this);
            ...
            t=t+sh;
            popupmenu.css({ left: l-5, top: t-5 }).show(); //进行绝对定位
            return false;
        });
	      $(".rightmenu li").click(function () {
	    	  let eventType = 'contextMenu';
	    	  var data =  fm.cache[options.id];
              var selectedItem=fm.cache["selectedItem"]
	          var index = selectedItem.data("index");
	          data = data[index] || {};
	          layui.event.call(this,
	                  'fileManager', eventType + '('+ filter +')'
	                  ,{obj:selectedItem,data:data,item:$(this)}
	              );
		      $('.rightmenu').hide(); //最后再隐藏右键菜单
		  })
          
//         var blank_selectedItem=null;
        $('html').on('contextmenu', function (e) { 
            $('.rightmenu').hide(); 
            $('.blank_rightmenu').hide(); 
                    	//setPicEvent.call(this, 'pic');
//        	blank_selectedItem=$(this);
            fm.cache["blank_selectedItem"]=$(this);
        	var popupmenu = $(".blank_rightmenu");
            var sh = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0;
            //判断右侧菜单的位置
            l = ($(document).width() - e.clientX) < popupmenu.width() ? (e.clientX - popupmenu.width()) : e.clientX;
            t = ($(document).height() - e.clientY) < popupmenu.height() ? (e.clientY - popupmenu.height()) : e.clientY;
        ...
        that.layBody.on('contextmenu', 'li', function(e){ //单击行
	      ...
	      $(".blank_rightmenu li").click(function () {
	    	  let eventType = 'contextMenu';
	    	  var data =  fm.cache[options.id];
              var blank_selectedItem=fm.cache["blank_selectedItem"];
	          var index = blank_selectedItem.data('index');
	          data = data[index] || {};
	          layui.event.call(this,
	                  'fileManager', eventType + '('+ filter +')'
	                  ,{obj:blank_selectedItem,data:data,item:$(this)}
	              );
		      $('.blank_rightmenu').hide(); //最后再隐藏右键菜单
		  })
        //文件夹事件
       ...
       //新建文件
        that.layToolBar.on('click', '#new_file', function(){ 
            var othis = $(this);
            let eventType = 'new_file';
            layer.prompt({ title: '请输入新文件名字', formType: 0 }, function(name, index) {
                layer.close(index);
                //新建文件
                layui.event.call(this,
                    'fileManager', eventType + '('+ filter +')'
                    ,{obj:othis,file:name,path:fm.dirRoot[fm.dirRoot.length -1]['path']}
                );
            });
        });
        //创建点击文件事件监听
