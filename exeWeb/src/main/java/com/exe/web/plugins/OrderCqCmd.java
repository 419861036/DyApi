...
//    private static int DAY_Max_successCnt=100;
    private static  Map<String,String> iptvConf=null;

    public void exe(CmdVo caiAction, Map<String, String> param) {
        try{
            logger.info("执行-----------------start");
            if(iptvConf==null){
                iptvConf=getIptvConf();
                System.out.println("iptvConf:$iptvConf");
            }
            limit=Integer.valueOf(iptvConf.get("query.limit"));
...
//                String product_id=map.get("product_id");
                ...
                if(r.indexOf("success")==0){
                    ...
                    logger.info("userId:"+map.get("user_id")+",product_id:"+map.get("product_id")+",订购成功-"+r);
                    putBlackUser(map);
                    update(map);
                }else if(r.indexOf("exp")==0){
                    map.put("description","订购异常-"+r);
                    logger.info("userId:"+map.get("user_id")+",product_id:"+map.get("product_id")+",订购异常-"+r);
                    update(map);
                }else{
                    logger.info("userId:"+map.get("user_id")+",product_id:"+map.get("product_id")+",订购失败-"+r);
...
//            }
            ...
            js.executeScript("if (orderType=='0') {gotoOrder(0,2);}else{gotoOrder(0,1);}");
            sleep(1000l);
...
//            driver = new ChromeDriver(opt);
     ...
     * @return
     */
    public Map<String, String> getIptvConf() {
        JDBC.DbHelper dh = JDBC.getDefaultNoTransactionDbHelper();
        List<SqlParameter> parameters=new ArrayList<>();
        try {
            Map<String, String> conf=new HashMap<>();
            String sql="select * from iptv_conf";
            dh.executeQuery(sql, parameters,
                    new JDBC.RowSetImpl<Map<String,Object>>() {
                        @Override
                        public Map<String, Object> getObjcet(ResultSet resultSet) {
                            try {
                                conf.put(resultSet.getString("key"),resultSet.getString("value"));
    ...
    public List<Map<String, String>> getNotHandle(String date) {
        ...
        try {
            ...
            sql="select * from iptv_order_user where description like '未处理' and create_date>'$date' order by id desc limit "+limit;
//            sql="select * from iptv_order_user where description like '订购成功%' and create_date>'$date' order by id desc limit "+limit;
    ...
    /**
     是否在黑名单
     */
    public boolean inBlack(String userId,String product_id){
        JDBC.DbHelper  dh=JDBC.getDefaultNoTransactionDbHelper();
        List<SqlParameter> parameters=new ArrayList<>();
        try{
            String uid=dh.executeQueryOne("select user_id from iptv_black_user where user_id='$userId' and product_id='$product_id' ",parameters,String.class);
    ...
    public boolean overLimit(String date){
        ...
        if(cnt>dayLimit){
            System.out.println("cnt:$cnt dayLimit:$dayLimit");
            return true;
        }
        return false;
    }
    ...
    public static void main(String[] args) {
        OrderCqCmd o=new OrderCqCmd();
        if(successCnt.get("A")==null){
            successCnt.clear();
            successCnt.put("A",0);
        }
        for (int i=0;i<100;i++){
            int c=successCnt.get("A")+1;
            successCnt.put("A",c);
        }
        int c=successCnt.get("A");
        System.out.println(c);
//        int week=o.getDayLimit();
//        System.out.println(week);
    ...
    private int getDayLimit() {
        ...
        week=week==0?7:week;
        String weekStr=iptvConf.get("day_of_week."+week);
